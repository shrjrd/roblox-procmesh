...
local function hashToList(s)     -- hashToList('1;2;3') -> 1, 2, 3
	local res = {}
	for str in gmatch(s, "([^,]+)") do
		insert(res, tonumber(str))
	end
	return unpack(res)
end
...
function m.linePointDistance(line, point)
	local segment = (line[2] - line[1]).unit --:normalize()
	--return segment:mul(segment:dot(point - line[1])):distance(point)
	return ((segment*(segment:Dot(point - line[1]))) - point).magnitude
end
...
function m:centerOfMass()
	...
	for i = 1, #self.vlist do
		...
		center = center + vec3(self.vlist[i][1], self.vlist[i][2], self.vlist[i][3])
	end
...
function m:volume()
	...
	for i = 1, #self.ilist, 3 do
		...
		local volume = abs(v1:Dot(v2:Cross(v3)) / 6)
...
function m:triangleToLine()
	...
	for _, line_pair in pairs(line_pairs) do
		insert(line_indices, line_pair[1])
		insert(line_indices, line_pair[2])
	end
	local other = self:clone()
...
function m.merge(solidsList)
	...
	for _, another in ipairs(solidsList) do
		...
		for _, index in ipairs(another.ilist) do
			insert(other.ilist, index + offset)
		end
...
function m:updateNormals()
	...
	for i = 1, #self.ilist, 3 do
		...
		v1 = vec3(unpack(self.vlist[vi1])) --v1:set(unpack(self.vlist[vi1]))
		v2 = vec3(unpack(self.vlist[vi2])) --v2:set(unpack(self.vlist[vi2]))
		v3 = vec3(unpack(self.vlist[vi3])) --v3:set(unpack(self.vlist[vi3]))
		--local fnormal = {v2:sub(v1):cross(v3:sub(v1)):normalize():unpack()}
		local _f = (v2-v1):Cross(v3-v1).unit
		local fnormal = {_f.x, _f.y, _f.z}
		normals[vi1] = normals[vi1] or {}
		normals[vi2] = normals[vi2] or {}
		normals[vi3] = normals[vi3] or {}
		insert(normals[vi1], fnormal)
		insert(normals[vi2], fnormal)
		insert(normals[vi3], fnormal)
	end
	local vnormal, tvec3 = vec3(), vec3()
	for i = 1, #self.vlist do
		assert(normals[i], 'no triangle in index list contains vertex ' .. i)
		if normals[i] then
			vnormal = vec3() --:set(0,0,0)
			local c = 0
			for _, fnormal in ipairs(normals[i]) do
				vnormal = vnormal + vec3(unpack(fnormal)) --:add(tvec3:set(unpack(fnormal)))
				c = c + 1
			end
			vnormal = vnormal * (1/c) --:mul(1 / c)
			local v = self.vlist[i]
			local _n = vnormal.unit
			v[4], v[5], v[6] = _n.x, _n.y, _n.z --vnormal:normalize():unpack()
		end
	end
...
function m.fromCSG(csg)
	...
	for _, p in ipairs(csg.polygons) do
		for j=3,#p.vertices do
			...
			insert(self.vlist, {v.pos.x, v.pos.y, v.pos.z, v.normal.x, v.normal.y, v.normal.z})
			local v = p.vertices[j-1]
			insert(self.vlist, {v.pos.x, v.pos.y, v.pos.z, v.normal.x, v.normal.y, v.normal.z})
			local v = p.vertices[j]
			insert(self.vlist, {v.pos.x, v.pos.y, v.pos.z, v.normal.x, v.normal.y, v.normal.z})
			insert(self.ilist, #self.ilist + 1)
			insert(self.ilist, #self.ilist + 1)
			insert(self.ilist, #self.ilist + 1)
		end
	end
...
function m.fromVertices(vertices, indices)
	...
	for i, vertex in ipairs(vertices) do
		...
		elseif type(vertex) == 'userdata' then
			...
			self.vlist[i] = {x, y, z}
		elseif type(vertex) == 'vector' then
			self.vlist[i] = {vertex.x, vertex.y, vertex.z}
		else
	...
	if indices then
		...
		local triangle_vertices = floor(#vertices / 3) * 3
...
function m.convexHull(point_cloud)
	if #point_cloud < 4 then
		...
		for i, point in ipairs(point_cloud) do
			vertices[i] = {point.x, point.y, point.z} --{ point:unpack() }
		end
	...
	while true do
		...
		tetrahedron = {}
		for i = 1, 4 do
			insert(tetrahedron, points[i])
		end
		if abs(m.trianglePointDistance(tetrahedron, tetrahedron[4])) > EPSILON then
			break
		else
			...
			for i = 1, 4 do -- take different set of 4 points for initial tetrahedron
				local rnd_index = math.random(5, #points)
	...
	local center = (tetrahedron[1] + tetrahedron[2] + tetrahedron[3] + tetrahedron[4])*(1/4) --:mul(1 / 4)
	for i = 1, 4 do
		local face = { i,
			1 + (i + 1) % 4,
			1 + (i + 2) % 4 }
		...
		if #conflicting_faces > 0 then -- add a conflict point to convex hull
			...
			for _, cface in ipairs(conflicting_faces) do
				for j = 1, 3 do
					...
					local hash = listToHash(min(i1, i2), max(i1, i2))
	...
	for _, face in ipairs(faces) do
		for _, index in ipairs(face) do
			if not input_to_hull[index] then
				insert(vertices, {points[index].x,points[index].y,points[index].z})--{points[index]:unpack()})
				input_to_hull[index] = #vertices
			end
			insert(indices, input_to_hull[index])
		end
	end
...
function m:addToConvexHull(point)
	insert(self.vlist, {point.x,point.y,point.z})--{point:unpack()})
	...
	local center = (vec3(unpack(self.vlist[1])) +
		...
		vec3(unpack(self.vlist[4])))*(1/4) --:mul(1 / 4)
	if #self.vlist == 4 then
		-- construct faces of intial convex hull with correct triangle windings
		for i = 1, 4 do
			local face = { i,
	...
	local faces = {}
	for i = 1, #self.ilist, 3 do
		insert(faces, {self.ilist[i], self.ilist[i + 1], self.ilist[i + 2]})
	end
	local conflicting_faces = {}
	for _, face in ipairs(faces) do
		local triangle = {vec3(unpack(self.vlist[face[1]])),
	...
	for _, face in ipairs(faces) do
		for _, index in ipairs(face) do
			if not input_to_hull[index] then
				insert(vertices, self.vlist[index])
				input_to_hull[index] = #vertices
			end
			insert(indices, input_to_hull[index])
		end
...
function m.superellipsePoints(segments, ax, ay, az, e1, e2)
	...
	for theta = -pi / 2, pi / 2, pi / segments do
		for gamma = -pi, pi, pi / segments do
			points[#points + 1] = vec3(
				ax * cos(theta)^e1 * abs(cos(gamma))^e2 * sgn(cos(gamma)),
				ay * cos(theta)^e1 * abs(sin(gamma))^e2 * sgn(sin(gamma)),
				az * abs(sin(theta))^e1 * sgn(sin(theta)))
		end
	end
	return points
end
...
function m.quad(subdivisions)
	local size = 1 / floor(subdivisions or 1)
	local self = m.new()
	local epsilon = 1e-6
	for y = -0.5, 0.5 - epsilon, size do
		for x = -0.5, 0.5 - epsilon, size do
			insert(self.vlist, {x, y, 0})
			insert(self.vlist, {x, y + size, 0})
			insert(self.vlist, {x + size, y, 0})
			insert(self.vlist, {x + size, y + size, 0})
...
function m.bipyramid(segments)
	...
	for i = 0, segments - 1 do
		...
		local x = 0.5 * cos(theta)
		local z = 0.5 * sin(theta)
		insert(self.vlist, {x, 0, z})
		insert(self.sides.ring, #self.vlist)
		theta = (i + 1) * (2 * pi) / segments
		x = 0.5 * cos(theta)
		z = 0.5 * sin(theta)
		insert(self.vlist, {x, 0, z})
...
function m.sphere(subdivisions)
	...
	for _ = 1, subdivisions do
		for i = #self.ilist, 1, -3 do
			...
			insert(self.ilist, v1)
			insert(self.ilist, a)
			insert(self.ilist, c)

			insert(self.ilist, v2)
			insert(self.ilist, b)
			insert(self.ilist, a)

			insert(self.ilist, v3)
			insert(self.ilist, c)
			insert(self.ilist, b)
			...
			remove(self.ilist, i - 0)
			remove(self.ilist, i - 1)
			remove(self.ilist, i - 2)
		end
	end
	-- Normalize
...
function m.sewindices(edgeA, edgeB, offsetA, offsetB)
	...
	for i = 1, #edgeA - 1 do
		...
		insert(indices, cA + offsetA) -- triangle I
		insert(indices, nB + offsetB)
		insert(indices, nA + offsetA)
		insert(indices, cA + offsetA) -- triangle II
		insert(indices, cB + offsetB)
		insert(indices, nB + offsetB)
	end
	return indices
end
...
local function tessellate_octasphere_patch(subdivisions)
	...
	local vertices = {}
	for i = 0, n - 1 do
		local theta = pi * 0.5 * i / (n - 1)
		point_a = vec3(0, sin(theta), cos(theta)) --:set(0, math.sin(theta), math.cos(theta))
		point_b = vec3(cos(theta), sin(theta), 0) --:set(math.cos(theta), math.sin(theta), 0)
	...
	for col_index = 0, n-2 do
		...
		for row = 0, col_height - 2 do
			insert(indices, j0 + row) -- edges and 'odd' inside faces
			insert(indices, j1 + row)
			insert(indices, j2 + row)
			insert(indices, j2 + row) -- 'even' inside faces
			insert(indices, j1 + row)
			insert(indices, j3 + row)
			f = f + 2
		end
		local row = col_height - 1
		insert(indices, j0 + row) -- faces with z = 0
		insert(indices, j1 + row)
		insert(indices, j2 + row)
		f = f + 1
		j0 = j2
...
function m.octasphere(subdivisions)
	...
	for ri, refl in ipairs(reflections) do
		reflect = vec3(unpack(refl)) -- reflect:set(unpack(refl))
		-- reflect/mirror the original patch, insert vertices into final shape
		for _,v in ipairs(patchvertices) do
			local rv = vec3(unpack(v)) --tvec3:set( unpack(v) )
			rv = rv + vec3(1,1,1) --:add(1,1,1) -- spread apart 8 domes, so that no two points are on same coordinates
			rv = rv * reflect --:mul(reflect)
			local x, y, z = rv.x, rv.y, rv.z --rv:unpack()
			local nv = vec3(unpack(v)) --tvec3:set( unpack(v) )
			nv = nv*reflect --:mul(reflect)
			local nx, ny, nz = nv.x, nv.y, nv.z --nv:unpack()
			insert(mergedvertices, { x, y, z, nx, ny, nz })
		end
